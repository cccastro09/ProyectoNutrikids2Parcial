    ...
    .then(usuarios => {
      Promise.all(
        usuarios.map(async usuario => {
          usuario.puntaje = await knex('partida').where({
            estado: 'a',
            idusuario: usuario.id
          })
          if (usuario.puntaje.length === 0) {
            usuario.puntaje = 0
          } else {
            usuario.puntaje = usuario.puntaje[0].puntaje
          }
          return usuario
        })
      ).then(usuariosFinal => {
        console.log(usuariosFinal)
        res.render('front/iniciar', { usuarios: usuariosFinal })
      })
    })
})
