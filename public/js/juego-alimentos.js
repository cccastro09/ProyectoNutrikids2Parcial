  ...
  var usuario=JSON.parse(localStorage.getItem("sesion"))
    ...
    function actualizarPartida(){
      ...
      // Petición HTTP GET síncrona hacia el archivo fotos.json del servidor
              ...
              req.open("POST", server+"/partidas/updatePartida", false);
              req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
              let partitm=partida;
              partitm=JSON.stringify(partitm)
              req.send("usuario="+partitm);
    }
    function crearPartida(){
      var req = new XMLHttpRequest();
      // Petición HTTP GET síncrona hacia el archivo fotos.json del servidor
               const server=window.location.origin;//atrapa ruta del servidor
              req.open("POST", server+"/partidas/crearPartida", false);
              req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
               partida={
              ...
              partidatmp=JSON.stringify(partidatmp)
              req.send("usuario="+partidatmp);
              ...
              localStorage.setItem("juego","inciada");
    }
 data=JSON.parse(localStorage.getItem("alimentos"));
    ...
    if(localStorage.getItem("juego")==undefined){
     crearPartida()
      ...
      // Petición HTTP GET síncrona hacia el archivo fotos.json del servidor
               const server=window.location.origin;//atrapa ruta del servidor
              req.open("GET", server+"/partidas/leerrPartidaActiva", false);
              req.send(null);
            ...
            if(partida.idusuario!=usuario.id){
              partida.estado="t"
              actualizarPartida()
              crearPartida()
            }else{
